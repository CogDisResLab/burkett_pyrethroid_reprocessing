...
pep_passed_qc <- krsa_quick_filter(data = data_pw_max, data2 = data_modeled$scaled,
...
differential_phosphorylated_peptides <- krsa_group_diff(data_modeled$scaled, c("B", "A"), pep_passed_qc, byChip = T)

# save LFC table
write_csv(differential_phosphorylated_peptides, "results/run-all-dpp_B_A-STK.csv")

# Extract top peptides based on the LFC cutoff using average of LFCs across chips
significant_peptides <-
  krsa_get_diff(differential_phosphorylated_peptides,
                totalMeanLFC ,
                c(0.15, 0.2, 0.3, 0.4)) %>% list("meanLFC" = .)
...
sigPeps_total <- list(significant_peptides, sigPepsPerChip) %>% unlist(recursive = F) %>%  unlist(recursive = F)
...
krsa_heatmap(data_modeled$normalized, significant_peptides$meanLFC$`0.2`, groups = c("B", "A"), scale = "row")
...
```{r}
#| label: violinIndPlotA
#| fig-width: 6
#| fig-height: 6
#| fig-cap: "Violin plot of two groups"

# generates a violin plot using the selected groups and peptides
krsa_violin_plot(data_modeled$scaled, significant_peptides$meanLFC$`0.2`, "Barcode", groups = c("B", "A"))
...
saveRDS(mutiple_krsa_outputs, file = "datastore/run-all-multiple_krsa_output_B_A_STK.RDS")
...
df <- df %>% rownames_to_column("method") %>% select(Kinase, Z, method) %>% 
...
AvgZTable2 |> 
  write_csv("results/run-all-krsa_table_full_B_A_STK.csv") |> 
  select(Kinase, AvgZ) |> 
  unique() |> 
  mutate(Direction = if_else(AvgZ >= 0, "Up", "Down")) |> 
  group_by(Direction) |> 
  nest() |> 
  mutate(filtered = map(data, \(x) {x |> mutate(AbsZ = abs(AvgZ)) |> arrange(desc(AbsZ)) |> slice_head(n = 10) |> select(-AbsZ)})) |> 
  select(-data) |> 
  unnest(filtered) |> 
  ungroup() |> 
  select(-Direction) |> 
